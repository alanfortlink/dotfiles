...
- **One line per interaction.** An interaction is a user prompt and
  ...
  running the line updates live, and the current activity is shown in pi's
  own "Working…" spinner row under the editor (`🧠 thinking…`, `💻 $ npm
  test`, `📖 file`, `writing…`) via `ui.setWorkingMessage` — one status row,
