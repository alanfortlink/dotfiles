...
- **Tool blocks** are capped at `TOOL_LINES` (10) visual lines while running
  ...
  keep the capped view so they stay visible.
...
pi has no hook for either, so `AssistantMessageComponent.updateContent` and
`ToolExecutionComponent.render` (both exported from the pi package) are
patched on their prototypes at load. Thinking Markdown children are wrapped in
...
```sh
ln -sfn /home/tank/repos/dotfiles/pi-extensions/compact-view \
