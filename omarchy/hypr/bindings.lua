...
-- Omarchy defaults on SUPER + jkl move to SUPER + SHIFT + jkl so SUPER + hjkl
-- is free for focus movement.
hl.unbind("SUPER + J")
hl.unbind("SUPER + K")
hl.unbind("SUPER + L")
...
o.bind("SUPER + H", "Focus left", "hypr-mux-focus l")
o.bind("SUPER + J", "Focus down", "hypr-mux-focus d")
o.bind("SUPER + K", "Focus up", "hypr-mux-focus u")
o.bind("SUPER + L", "Focus right", "hypr-mux-focus r")
...
-- === Fullscreen ===
-- SUPER+F was: Full screen (omarchy default); ALT+SHIFT+F below covers that.
hl.unbind("SUPER + F")
o.bind("SUPER + F", "Maximize (full width)", hl.dsp.window.fullscreen({ mode = "maximized" }))
-- In herdr/tmux (with >1 pane) this toggles pane zoom instead.
o.bind("SUPER + Z", "Maximize / mux pane zoom", "hypr-mux-zoom")
...
-- keeps the bar visible. So hop the window out of the scratchpad before
...
-- === Scratchpad ===
-- Only the one generic scratchpad; the numbered scratch1-4 are gone.
-- Pin it to the right monitor (DP-1) so focusing back to it doesn't
-- migrate/hide the special workspace.
hl.workspace_rule({ workspace = "special:scratchpad", monitor = "DP-1" })
hl.workspace_rule({ workspace = "special:scratchpad2", monitor = "DP-1" })
...
local function toggle_scratchpad(name)
  return 'hyprctl --batch \'dispatch hl.dsp.focus({ monitor = "DP-1" }) ; dispatch hl.dsp.workspace.toggle_special("'
    .. name
    .. '")\''
end

hl.unbind("SUPER + S")
o.bind("SUPER + S", "Toggle scratchpad", toggle_scratchpad("scratchpad"))
o.bind("ALT + S", "Toggle window in/out of scratchpad", "hypr-scratchpad-out")

-- Second scratchpad on the grave key, same behaviour as S.
o.bind("SUPER + grave", "Toggle scratchpad 2", toggle_scratchpad("scratchpad2"))
o.bind("ALT + grave", "Toggle window in/out of scratchpad 2", "hypr-scratchpad-out scratchpad2")

-- CTRL+SHIFT+F1..F5 switch workspaces (were: scratchpads 1-4); the ALT
-- variant sends the focused window there. Both always act on the right
-- monitor (LG / DP-1) so the left one (Samsung) never changes workspace:
-- focus DP-1 first, then select the workspace, which pulls it over if it
-- happened to live elsewhere.
local RIGHT_MONITOR = "DP-1"

-- Hide the scratchpad if it's showing on the right monitor.
local function hide_right_special()
  for _, m in ipairs(hl.get_monitors()) do
    if m.name == RIGHT_MONITOR then
      local sw = m.active_special_workspace
      if sw and sw.name ~= "" then
        hl.dispatch(hl.dsp.workspace.toggle_special(sw.name:gsub("^special:", "")))
      end
      return
    end
  end
...
local function show_on_right(ws)
  hl.dispatch(hl.dsp.focus({ monitor = RIGHT_MONITOR }))
  hide_right_special()
  hl.dispatch(hl.dsp.focus({ workspace = ws }))
end

-- On the main keyboard F1..F4 are remapped in firmware to F14..F17, so both
-- the plain F14..F17 (and ALT+F14..F17) and the CTRL+SHIFT+F1..F5 (and
-- CTRL+SHIFT+ALT+F1..F5) forms are bound, for other keyboards.
local function bind_workspace(switch_key, move_key, ws)
  o.bind(switch_key, "Switch to workspace " .. ws .. " (right monitor)", function()
    show_on_right(ws)
  end)
  o.bind(move_key, "Move window to workspace " .. ws .. " (right monitor)", function()
    hl.dispatch(hl.dsp.window.move({ workspace = ws, follow = false }))
    show_on_right(ws)
  end)
end

-- xkeyboard-config maps the F14..F17 keycodes to XF86Launch5..8 keysyms
-- (not F14..F17), so bind those too.
for i, ws in ipairs({ "1", "3", "4", "5", "6" }) do
  bind_workspace("CTRL + SHIFT + F" .. i, "CTRL + SHIFT + ALT + F" .. i, ws)
  if i <= 4 then
    bind_workspace("F" .. (i + 13), "ALT + F" .. (i + 13), ws)
    bind_workspace("XF86Launch" .. (i + 4), "ALT + XF86Launch" .. (i + 4), ws)
  end
end
...
o.bind("ALT + L", "Address bar", mac_routed("L"))
o.bind("ALT + F", "Find", mac_routed("F"))
-- Ctrl/Shift+Insert copy/paste are universal and work in terminals too, so
-- ALT+C / ALT+V are copy/paste everywhere, including inside herdr/tmux. Mux
-- new-tab / splits live on the ctrl+shift direct layer (ctrl+shift+t/o/e).
...
-- ALT+SHIFT+1..9 inject ctrl+alt+<digit>: herdr's switch_workspace direct
-- chord (workspace 1-9). Hyprland must not own these, and alt+[ / alt+]
-- (herdr previous/next workspace) stay unbound here so the chords pass
-- through untouched.
for i = 1, 9 do
  o.bind("ALT + SHIFT + " .. i, "mux workspace " .. i, mac("CTRL ALT", tostring(i)))
end
o.bind("ALT + 0", "Reset zoom", mac("CTRL", "0"))
o.bind("ALT + A", "Select all", mac("CTRL", "A"))
-- Editing/document shortcuts: ALT -> CTRL, universal combos.
-- (Cmd+S/Save is NOT mapped: ALT+S is the scratchpad toggle. Undo is NOT
-- mapped; use the app's native Ctrl+Z.)
...
o.bind("ALT + equal", "Zoom in (agent in terminal)", mac_routed("equal"))
o.bind("ALT + minus", "Zoom out (agent in terminal)", mac_routed("minus"))
